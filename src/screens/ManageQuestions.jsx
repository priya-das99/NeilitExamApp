...
const ManageQuestions = () => {
  ...
  }, [searchQuery, selectedTags, questions]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView horizontal style={styles.tagsScrollView} showsHorizontalScrollIndicator={false}>
        <View style={styles.tagsContainer}>
          {allTags.map(tag => (
            <TouchableOpacity
              key={tag}
        ...
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent}>
            {questionForms.map((q, idx) => (
              <View key={idx} style={{ marginBottom: 32, borderBottomWidth: 1, borderColor: '#eee', paddingBottom: 16 }}>
                <Text style={styles.questionNumberLabel}>Question {questions.length + idx + 1}</Text>
                <Text style={styles.label}>Question Type *</Text>
                <View style={styles.typeContainer}>
                  {['mcq', 'msq'].map(type => (
                    <TouchableOpacity
                      key={type}
                      onPress={() => handleFormChange(idx, 'type', type)}
                      style={[
                        styles.typeButton,
                        q.type === type && styles.typeButtonActive
                      ]}
                    >
                      <Text style={[
                        styles.typeButtonText,
                        q.type === type && styles.typeButtonTextActive
                      ]}>
                        {type.toUpperCase()}
                      </Text>
                    </TouchableOpacity>
                ...
                <TextInput
                  style={styles.input}
                  value={q.text}
                  onChangeText={text => handleFormChange(idx, 'text', text)}
                ...
                <View style={styles.imageContainer}>
                  {q.image ? (
                    <View style={styles.imagePreview}>
                      <Image source={{ uri: q.image.uri }} style={styles.previewImage} />
                      <TouchableOpacity onPress={() => handleRemoveImage(idx)} style={styles.removeImageButton}>
                        <Icon name="close" size={20} color="#fff" />
                      </TouchableOpacity>
                    </View>
                  ) : (
                    <TouchableOpacity onPress={() => handleImagePick(idx)} style={styles.uploadButton}>
                ...
                {q.options.map((option, optIdx) => (
                  <View key={optIdx} style={styles.optionContainer}>
                    <TouchableOpacity
                      onPress={() => handleCorrectAnswerChange(idx, optIdx)}
                    ...
                    <TextInput
                      ...
                      value={option.text}
                      onChangeText={text => handleOptionChange(idx, optIdx, text)}
                      placeholder={`Option ${String.fromCharCode(65 + optIdx)}`}
                    />
                    {option.image ? (
                      <View style={styles.optionImageContainer}>
                        <Image source={{ uri: option.image.uri }} style={styles.optionImage} />
                        <TouchableOpacity onPress={() => handleRemoveOptionImage(idx, optIdx)} style={styles.removeOptionImageButton}>
                          <Icon name="close" size={16} color="#fff" />
                        </TouchableOpacity>
                      </View>
                    ) : (
                      <TouchableOpacity onPress={() => handleOptionImagePick(idx, optIdx)} style={styles.optionImageButton}>
                ...
                <View style={styles.difficultyContainer}>
                  {['easy', 'medium', 'hard'].map(level => (
                    <TouchableOpacity
                      key={level}
                      onPress={() => handleFormChange(idx, 'difficulty', level)}
                      style={[
                        styles.difficultyButton,
                        q.difficulty === level && styles.difficultyButtonActive
                      ]}
                    >
                      <Text style={[
                        styles.difficultyButtonText,
                        q.difficulty === level && styles.difficultyButtonTextActive
                      ]}>
                ...
                <TextInput
                  style={styles.input}
                  value={q.tags}
                  onChangeText={text => handleFormChange(idx, 'tags', text)}
                ...
                <TextInput
                  ...
                  onChangeText={text => handleFormChange(idx, 'marks', text.replace(/[^0-9]/g, ''))}
                  placeholder="Enter marks"
                  keyboardType="numeric"
                />
                {questionForms.length > 1 && (
                  <TouchableOpacity onPress={() => handleRemoveQuestionForm(idx)} style={[styles.button, styles.cancelButton, { alignSelf: 'flex-end', marginTop: 8, marginBottom: 0 }]}> 
                    <Text style={styles.buttonText}>Remove</Text>
                  </TouchableOpacity>
                )}
              </View>
            ))}
            <TouchableOpacity onPress={handleAddQuestionForm} style={[styles.button, styles.saveButton, { marginBottom: 16 }]}>
          ...
          <View style={styles.modalFooter}>
            ...
            <TouchableOpacity onPress={handleSaveAll} style={[styles.button, styles.saveButton]} disabled={saving}>
              {saving ? (
